...
impl PageView {
    ...
    pub fn scroll_to_page(&mut self, index: usize) {
        ...
        match direction {
            ScrollDirection::HORIZONTAL => {
                ...
                self.scroll_view
                    .set_inner_container_position(Vec2::new(dest_x, 0.0));
            }
            ScrollDirection::VERTICAL => {
                let page_height = self.scroll_view.get_widget().get_size().y;
                let dest_y = -(index as f32 * page_height);
                self.scroll_view
                    .set_inner_container_position(Vec2::new(0.0, dest_y));
            }
            _ => {}
        }
    ...
    pub fn scroll_to_page_with_time(&mut self, index: usize, time: f32) {
        ...
        match direction {
            ScrollDirection::HORIZONTAL => {
                ...
                self.scroll_view
                    .scroll_to_percent_horizontal(percent, time, true);
            }
            ScrollDirection::VERTICAL => {
                let percent = (index as f32 / self.pages.len() as f32) * 100.0;
                self.scroll_view
                    .scroll_to_percent_vertical(percent, time, true);
            }
            _ => {}
        }
    ...
    fn update_pages_layout(&mut self) {
        ...
        match direction {
            ScrollDirection::HORIZONTAL => {
                ...
                self.scroll_view
                    .set_inner_container_size(Vec2::new(total_width, container_size.y));
            }
            ...
            ScrollDirection::VERTICAL => {
                ...
                self.scroll_view
                    .set_inner_container_size(Vec2::new(container_size.x, total_height));
            }
    ...
    fn check_page_alignment(&mut self) {
        ...
        let current_index = match direction {
            ScrollDirection::HORIZONTAL => {
                let page_width = self.scroll_view.get_widget().get_size().x;
                ((-position.x / page_width).round() as usize)
                    .min(self.pages.len().saturating_sub(1))
            }
            ScrollDirection::VERTICAL => {
                let page_height = self.scroll_view.get_widget().get_size().y;
                ((-position.y / page_height).round() as usize)
                    .min(self.pages.len().saturating_sub(1))
            }
            _ => 0,
        };
