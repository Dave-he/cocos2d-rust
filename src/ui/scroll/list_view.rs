    ...
    pub fn scroll_to_item(&mut self, index: usize, time_in_sec: f32, attenuated: bool) {
        ...
        match direction {
            ScrollDirection::VERTICAL => {
                ...
                self.scroll_view
                    .scroll_to_percent_vertical(percent, time_in_sec, attenuated);
            }
            ScrollDirection::HORIZONTAL => {
                let percent = (index as f32 / self.items.len() as f32) * 100.0;
                self.scroll_view
                    .scroll_to_percent_horizontal(percent, time_in_sec, attenuated);
            }
            _ => {}
        }
    }
    ...
    fn refresh_view(&mut self) {
        ...
        match direction {
            ScrollDirection::VERTICAL => {
                ...
                let container_width = self.scroll_view.get_widget().get_size().x;
                self.scroll_view
                    .set_inner_container_size(Vec2::new(container_width, total_size));
            }
            ...
            ScrollDirection::HORIZONTAL => {
                ...
                let container_height = self.scroll_view.get_widget().get_size().y;
                self.scroll_view
                    .set_inner_container_size(Vec2::new(total_size, container_height));
            }
