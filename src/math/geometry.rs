...
impl Rect {
    ...
    pub fn contains_point(&self, point: &Vec2) -> bool {
        point.x >= self.get_min_x()
            && point.x <= self.get_max_x()
            && point.y >= self.get_min_y()
    ...
    pub fn intersects_circle(&self, center: &Vec2, radius: f32) -> bool {
        ...
        if closest_point.x < min_x {
            closest_point.x = min_x;
        } else if closest_point.x > max_x {
            closest_point.x = max_x;
        }

        if closest_point.y < min_y {
            closest_point.y = min_y;
        } else if closest_point.y > max_y {
            closest_point.y = max_y;
        }
