use super::device::{
    BufferObject, BufferType, BufferUsage, FramebufferObject, GraphicsDevice, ShaderProgram,
    TextureObject,
};
...
impl OpenGLBackend {
    ...
    pub fn attach_shader(&self, program: u32, shader: u32) {
        let prog_native = self
            .device
            .get_program(program)
            .and_then(|p| p.get_native());
    ...
    pub fn set_uniform_matrix4fv(
        ...
        if self.current_program != 0 {
            if let Some(native_loc) = self
                .device
                .get_native_uniform(self.current_program, location)
            {
                ...
                unsafe {
                    self.device.context.uniform_matrix_4_f32_slice(
                        Some(&native_loc),
                        transpose,
                        &slice,
                    )
    ...
    pub fn buffer_data(
        ...
        unsafe {
            self.device
                .context
                .buffer_data_u8_slice(target, data, usage)
        };
    }
    ...
    pub fn draw_elements(&self, mode: u32, count: i32, type_: u32, offset: i32) {
        unsafe {
            self.device
                .context
                .draw_elements(mode, count, type_, offset)
        };
    }
    ...
    pub fn tex_image_2d(
        ...
        unsafe {
            self.device.context.tex_image_2d(
                target,
                level,
                internal_format,
                width as i32,
    ...
    pub fn tex_sub_image_2d(
        ...
        unsafe {
            self.device.context.tex_sub_image_2d(
                target,
                level,
                x_offset,
                y_offset,
    ...
    pub fn set_texture_params(&self, min_filter: u32, mag_filter: u32, wrap_s: u32, wrap_t: u32) {
        ...
        unsafe {
            self.device.context.tex_parameter_i32(
                glow::TEXTURE_2D,
    ...
    pub fn framebuffer_texture_2d(
        ...
        if let Some(tex) = self.device.get_texture(texture) {
            if let Some(native) = tex.get_native() {
                unsafe {
                    self.device.context.framebuffer_texture_2d(
                        target,
                        attachment,
                        tex_target,
                        Some(native),
