            ...
            if let Some(plist::Value::Dictionary(frames)) = root.get("frames") {
                for (frame_name, frame_data) in frames.iter() {
                    if let plist::Value::Dictionary(frame_dict) = frame_data {
                        // 解析帧矩形
                        let rect =
                            if let Some(plist::Value::String(rect_str)) = frame_dict.get("frame") {
                        ...
                        // 解析是否旋转
                        let rotated =
                            if let Some(plist::Value::Boolean(r)) = frame_dict.get("rotated") {
                                *r
                            } else {
                                false
                            };
                        ...
                        // 解析原始尺寸
                        let original_size = if let Some(plist::Value::String(size_str)) =
                            frame_dict.get("sourceSize")
                        {
                        ...
                        // 解析偏移
                        let offset = if let Some(plist::Value::String(offset_str)) =
                            frame_dict.get("offset")
                        {
...
mod tests {
    ...
    fn test_shared_instance() {
        ...
        cache1
            .borrow_mut()
            .add_frame(SpriteFrame::new("shared_frame"));
